...
class player(GameSprite):
    def update_r(self):
        ...
        if keys_pres[K_UP] and self.rect.y > 5:
            self.rect.y -= self.speed
        if keys_pres[K_DOWN] and self.rect.y < win_h - 80:
            self.rect.y += self.speed
    def update_l(self):
        keys_pres = key.get_pressed()
        if keys_pres[K_w] and self.rect.y > 5:
            self.rect.y -= self.speed
        if keys_pres[K_s] and self.rect.y < win_h - 80:
            self.rect.y += self.speed
